...
async def predict_from_file(file: UploadFile = File(...), engine_type: str = Form(...), row_index: int = Form(0)):
    ...
    if extension == "csv":
        ...
        try:
            ...
            return JSONResponse(status_code=500, content={"error": "Missing 'infer_column_schema' function from training module."})
    else:
...
if __name__ == "__main__":
    ...
    if args.serve:
        ...
        try:
            ...
            save_name = f"reports/shap_force_plot_{args.engine_type.lower()}.png"
            save_shap_force_plot(result["explanation"], save_path=save_name)
